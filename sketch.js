...
var awmSound, awmSound2, cSound, gSound, guSound;
...
function draw() {
  ...
  booyahText.position(300,150);
  booyahText.style('font-size', '35px'); 
  booyahText2.position(450,200);
  booyahText2.style('font-size', '25px');

  defeatText.position(300,150);
  defeatText.style('font-size', '35px'); 
  defeatText2.position(450,200);
...
function shoot() {
  ...
  if (keyDown("space")) {
    ...
    if (frameCount % 9 === 0) {
      ...
      gSound.play();
      gSound.setVolume(0.1);
      bulletGroup.add(bullet);
      bulletGroup.setLifetimeEach(75);
    }
  }
}
...
function enemyShoot() {
  ...
  if(enemy5.x <= 1300){
    if(frameCount % 60===0){
      ...
      enemyBullet5.velocityX = -8;
      awmSound2.play();
      awmSound2.setVolume(0.07);
...
function MedKit() {
  ...
  if (keyDown("m") && medCount == 1) {
    ...
    playerHealthBar.width = playerHealthBar.width + 50;
...
function instructions() {
  ...
  text("3.YOU WILL GET ONLY 2-3 MEDKITS DURING THE GAME", 30, 200);
  text("4.ALL YOUR GUNS GIVE THE SAME DAMAGE", 30, 250);
  text("GOOD LUCK:-)", 400, 350);
